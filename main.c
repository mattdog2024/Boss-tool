 ...
 *     网卡与主线程 ApplyIP 并发执行，导致 IP 被应用到正在 down 的网卡上
 *     → IP 丢失 → IPGuardThread 检测到"IP丢失"→ 重新应用 → ~1秒闪断
 *   - 修复 1：MAC 随机化改回同步执行（在 BeginNetworkChange 之前），
 *     确保 MAC 完成且网卡稳定后才应用 IP，彻底消除竞态条件
 *   - 修复 2：IPGuardThread 轮询间隔 15s → 30s，减少不必要的检查
 *   - 修复 3：IP 丢失确认等待 30s → 60s，给适配器更充分的恢复时间
 *   - 修复 4：IPGuardThread 重新应用 IP 前检查适配器操作状态
 *     (OperStatus)，只在适配器完全 up 时才执行恢复
 *   - 修复 5：IPGUARD_COOLDOWN_MS 60s → 120s，防止 MAC 后过早触发恢复
 *
...
/* v4.3: IPGuard 冷却 — RandomizeMac 后不触发IP恢复
 * v4.20: 60秒→120秒，MAC 随机化后网卡+TCP/IP 栈需要更长时间稳定，
 * 过早检查会误判"IP丢失"→ 触发不必要的恢复 → 网络闪断 */
static volatile LONG g_dwLastNetworkResetTick = 0;
#define IPGUARD_COOLDOWN_MS (120 * 1000)  /* v4.20: 120秒冷却 */
...
DWORD WINAPI IPGuardThread(LPVOID p) {
    ...
    while (1) {
        Sleep(30000);  /* v4.20: 15秒→30秒，减少 GetAdaptersInfo 调用和网络栈压力 */
        ...
        if (!AdapterHasIP(g_szExpectedIP)) {
            Sleep(60000);  /* v4.20: 30秒→60秒，给适配器更充分的恢复时间 */
            if (g_bAllowIPChange || g_szExpectedIP[0] == 0) continue;
            if (g_lNetworkChangeBusy || AdapterHasIP(g_szExpectedIP)) continue;

            /* v4.3: 再次检查冷却（60秒等待期间可能已满足冷却条件） */
            {
                DWORD dwNow = GetTickCount();
                DWORD dwReset = (DWORD)g_dwLastNetworkResetTick;
                if (dwReset != 0 && (dwNow - dwReset) < (DWORD)IPGUARD_COOLDOWN_MS) {
            ...
            /* v4.20: 检查适配器操作状态 — 只在适配器完全 up 时才恢复
             * 避免适配器还在初始化时执行 ClearAccumulatedIPs + ApplyIP
             * 导致网络闪断（~1秒断连） */
            if (!IsAdapterOperational()) {
                WriteLog(L"IPGuardThread: 适配器未就绪（OperStatus!=1），跳过本次恢复");
                continue;
            }
...
static void SetIPWork(void) {
    /* v4.20: MAC 随机化改回同步执行（在 BeginNetworkChange 之前）。
     *
     * v4.4 将 MAC 移至后台线程以加速老板键响应，但引入了竞态条件：
     * MAC 线程 disable/enable 网卡与主线程 ApplyIP 并发执行，导致：
     *   1. IP 被应用到正在 down 的网卡 → IP 丢失
     *   2. IPGuardThread 检测到"IP丢失"→ 重新应用 → ~1秒网络闪断
     *
